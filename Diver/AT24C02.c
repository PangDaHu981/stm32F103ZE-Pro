...
u8 AT24C02_WriteByte(u8 addr, u8 wdata) {
    ...
    if (IIC_Wait_Ack(100) != 0) {
        return 1;   // 等待应答信号如果出错，则返回
    }
    IIC_Send_Byte(addr);   // 写存储地址
    if (IIC_Wait_Ack(100) != 0) {
        return 2;   // 等待应答信号如果出错，则返回
    }
    IIC_Send_Byte(wdata);   // 写数据
    if (IIC_Wait_Ack(100) != 0) {
        return 3;   // 等待应答信号如果出错，则返回
    }
    IIC_Stop();
    return 0;
}
...
u8 AT24C02_ReadData(u8 addr, u8 *rdata, u16 datalen) {
    ...
    IIC_Start();
    IIC_Send_Byte(AT24C02_RADDR); // 读设备地址
    if (IIC_Wait_Ack(100) != 0) {
        IIC_Stop();
        return 3; // 等待应答信号出错
    }
    ...
    for (u16 i = 0; i < datalen; i++) {
        if (i == datalen - 1) {
            rdata[i] = IIC_Read_Byte(0); // 最后一个字节发送 NACK
        } else {
            rdata[i] = IIC_Read_Byte(1); // 发送 ACK
        }
    }
